...
class ABSDataset(Dataset):
    def __init__(self, images_dir, transforms=(), dataset_name='', domain=0):
        self.images_dir = images_dir
        self.transforms_pms = transforms
    ...
    def __getitem__(self, idx):
        ...
        if self.is_sample == 1 and self.domain == 0:
    ...
    def record_norm_info(self):
        norm_pms = [0.0] * 8
        transforms_lpms = None
        for tm in self.transforms_pms:
            if 'mean' in tm.keys():
               transforms_lpms = tm
               break
               
        if 'mean' in transforms_lpms.keys():
            norm_pms[0:3] = transforms_lpms['mean']
        
        if 'std' in transforms_lpms.keys():
            norm_pms[3:6] = transforms_lpms['std']
        
        if 'to_01' in transforms_lpms.keys():
            norm_pms[6] = 1.0 if transforms_lpms['to_01'] else 0.0
            
        if 'to_bgr' in transforms_lpms.keys():
            norm_pms[7] = 1.0 if transforms_lpms['to_bgr'] else 0.0
...
class VOCDataset(ABSDataset):
    ...
    CLASSES = (False, True, True, True, True,
