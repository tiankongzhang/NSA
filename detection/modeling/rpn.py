...
class RPN(nn.Module):
    ...
    def forward(self, images, features, img_metas, targets=None):
        ...
        with torch.no_grad():
            proposals, bbox_pd = self.generate_proposals(anchors, logits, bbox_reg, img_metas, is_target_domain)
        ...
        return proposals, loss, logits, bbox_pd
    ...
    def generate_proposals(self, anchors, objectness, box_regression, img_metas, is_target_domain=False):
        ...
        box_pds = box_pds.view(N, H, W, A*4)
        box_pds = box_pds.permute(0, 3, 1, 2)
        return results, box_pds
